...
//	}
	/**
	 * Recurring host reports (the generator propagation summary) all register under this key, so a new
	 * one closes the previous one instead of stacking up night after night, and no unrelated popup can
	 * close it - see FrameManager.showDefaultManagedFrame(..., frameKey).
	 */
	public static final String REPORT_FRAME_KEY = "hostReportWithAbout";
	/**
	...
	public static void popupMessage(JFrame parent,RealmDirectInfoHolder info) {
		...
		if (aboutText!=null && aboutText.trim().length()>0) {
			...
			/*
			 * NON-MODAL, and deliberately so even when there is no parent frame.  This is a report a
			 * player reads at leisure while carrying on with the game, not a question, and the old
			 * JOptionPane path for a null parent locked the EDT until somebody clicked it.  The About
			 * dialog it opens IS modal, but only over this frame.
			 *
			 * Registered under REPORT_FRAME_KEY so the next report disposes this one first - otherwise
			 * a fresh window arrives every day and they pile up unread.
			 */
			FrameManager.showDefaultManagedFrame(parent,content,title,icon,true,null,REPORT_FRAME_KEY);
			return;
		}
		if (parent==null) {
