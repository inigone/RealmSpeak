...
public class RealmHostPanel extends JPanel {
	...
	private void updateGameStateRecording(RealmCalendar cal) { // BIRDSONG
		...
		RealmObjectMaster rom = RealmObjectMaster.getRealmObjectMaster(host.getGameData());
		rom.resetDenizenObjects();
		ArrayList<GameObject> denizenObjects = rom.getDenizenObjects();
		for (GameObject go:denizenObjects) {
			...
			if (blownSpellId!=null || hurricaneWindsEvent || go.hasThisAttribute(Constants.LAND_FIRST)) {
				...
				if (!rc.getGameObject().hasThisAttribute(Constants.DEAD) && !rc.isCharacter() && !rc.isHiredLeader()) {
					// Land now!
...
//						actionFollower.setCombatPlayOrder(playOrder);
	...
	private void updateGameStateMidnight(RealmCalendar cal) {
		 ...
		 * propagating (SetupCardUtility.chooseGeneratedMonsterMove returns immediately on isBlocked,
		 * and under MONSTER PODS a whole pod holds if any one member is blocked) and prowling (the
		 * prowl loop in summonMonsters excludes blocked monsters) for the rest of the game.
		 * Unblocking a monster is harmless whichever content set it came from, and the isMonster()
		 * test below still keeps non-monsters out.
		 */
	...
	private void runDailyGeneratorPhase(DieRoller monsterDieRoller,DieRoller nativeDieRoller) {
		String report = SetupCardUtility.runDailyGeneratorPhase(hostPrefs,host.getGameData(),monsterDieRoller,nativeDieRoller);
		if (report==null) return;
		// Broadcast first: this is the permanent record, and it survives the dialog being dismissed
		// (or never arriving, if a client is mid-reconnect).
		...
		popupToAllPlayers("Daily Propagation",report,SetupCardUtility.getPropagationAboutText(hostPrefs));
	}
	/**
